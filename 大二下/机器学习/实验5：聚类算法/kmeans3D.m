...
class=zeros(1,k);
classCenter=[2 8 8;3 9 9;4 10 10];
figure(1);
scatter3(classCenter(:,1),classCenter(:,2),classCenter(:,3),[],classColor,'filled');
hold on;
scatter3(sample(:,1),sample(:,2),sample(:,3),'m');
for iterator=1:3
    nextCenter=[0 0 0;0 0 0;0 0 0];
    classNumber=[0 0 0];
    for i=1:sampleNumberAll
        distances=zeros(1,k);
        for j=1:k
            distances(j)=pdist([sample(i,:);classCenter(j,:)]);
        end
    ...
    for i=1:sampleNumberAll
        if class(i)==1
            scatter3(sample(i,1),sample(i,2),sample(i,3),'r','s');
        elseif class(i)==2
            scatter3(sample(i,1),sample(i,2),sample(i,3),'g','d');
        else
            scatter3(sample(i,1),sample(i,2),sample(i,3),'b','h');
        end
    end
end
